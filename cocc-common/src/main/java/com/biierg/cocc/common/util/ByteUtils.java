...
public class ByteUtils {
	 ...
	 * @return
	 */
	public static byte[] getBytes4Bigendian(long longVal, byte[] valBytes) {

		for (int i = 0; valBytes != null && i < valBytes.length; i++) {
			valBytes[valBytes.length - 1 - i] = (byte) ((longVal >>> (8 * i)) & 0xFF);
		}
	...
	public static long getUnsignedInt4Bigendian(byte[] data, int fromIdx, int toIdx) {
		...
		if (data == null || fromIdx > toIdx || toIdx > data.length) {
			throw new IllegalArgumentException("arguments is invalid fromIdx:" + fromIdx + ", toIdx:" + toIdx);
		}
		if (toIdx - fromIdx > 4) {
			throw new IllegalArgumentException("arguments is invalid");
		}
